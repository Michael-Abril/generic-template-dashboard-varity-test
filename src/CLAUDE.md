...
Focus is on testing OAuth flows for these 6 integrations at https://app.varity.so/marketplace:

| Integration | OAuth Flow | Data Sync | Status |
|-------------|:----------:|:---------:|--------|
| **QuickBooks** | ✅ Works | 🔴 403 (App in Dev Mode) | Needs Intuit approval |
| **Google Workspace** | ❓ Test | ❓ Test | Ready for testing |
| **Microsoft 365** | ❓ Test | ❓ Test | Ready for testing |
| **Slack** | ❓ Test | ❓ Test | Ready for testing |
| **Salesforce** | ❓ Test | ❓ Test | Ready for testing |
| **HubSpot** | ❓ Test | ❓ Test | Ready for testing |
...
Test at https://app.varity.so/marketplace - All 6 backend credentials are configured.

**Testing Checklist for Each Provider:**
- [ ] QuickBooks - OAuth ✅, Data Sync 🔴 (App in Dev Mode)
- [ ] Google Workspace - Test OAuth flow
- [ ] Microsoft 365 - Test OAuth flow
- [ ] Slack - Test OAuth flow
- [ ] Salesforce - Test OAuth flow
- [ ] HubSpot - Test OAuth flow

**Previous Bug Fixes Applied (Dec 13, 2025):**

| Bug | Fix | File |
|-----|-----|------|
| `invalid_grant` error (OAuth codes used twice) | Added `hasProcessed` flag | `app/oauth/callback/[provider]/page.tsx` |
| QuickBooks `realmId` missing | Extract from URL, pass to backend | `app/oauth/callback/[provider]/page.tsx` |
