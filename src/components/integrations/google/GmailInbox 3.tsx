...
export function GmailInbox({ walletAddress, data }: GmailInboxProps) {
  ...
  const handleReply = (email: Email, type: 'reply' | 'replyAll' | 'forward' = 'reply') => {
    // Store reply context for the composer
    const replyTo = type === 'forward' ? '' : email.from;
    const subject = type === 'forward'
      ? `Fwd: ${email.subject}`
      : email.subject.startsWith('Re:') ? email.subject : `Re: ${email.subject}`;

    // In a full implementation, we'd pass this context to the EmailComposer
    // For now, just show the composer
